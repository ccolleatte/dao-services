#!/usr/bin/env python3
"""
Query Set 6: Complementary Queries
Fill gaps to reach 135 papers minimum target.
Focus: Foundational papers, blockchain-specific cryptoeconomics, survey papers.
"""
...
# Query Set 6 definitions - Complementary queries to reach 135 minimum
QUERIES = [
    {
        "query_id": "6A",
        "description": "Foundational DAO papers (2020-2021)",
        "query_string": "cat:cs.CY OR cat:cs.CR AND (ti:DAO OR ti:\"decentralized autonomous organization\" OR ti:\"blockchain governance\") AND submittedDate:[2020 TO 2021]",
        "max_results": 20
    },
    {
        "query_id": "6B",
        "description": "Proof-of-Stake and Validator Economics",
        "query_string": "cat:cs.CR OR cat:econ.TH AND (ti:\"proof of stake\" OR ti:\"proof-of-stake\" OR ti:validator OR ti:\"staking mechanism\") AND abs:incentive AND submittedDate:[2022 TO 2025]",
        "max_results": 15
    },
    {
        "query_id": "6C",
        "description": "Survey/SoK papers on blockchain governance and consensus",
        "query_string": "(ti:survey OR ti:SoK OR ti:\"systematic review\") AND (abs:blockchain OR abs:\"distributed consensus\") AND (abs:governance OR abs:consensus OR abs:voting) AND submittedDate:[2020 TO 2025]",
        "max_results": 20
    }
]
...
def score_paper(paper: Dict, query_context: str) -> Dict:
    ...
    topic_keywords = {
        "6A": ["dao", "decentralized autonomous", "governance", "blockchain governance", "on-chain"],
        "6B": ["proof of stake", "validator", "staking", "slashing", "consensus", "pos"],
        "6C": ["survey", "sok", "review", "consensus", "governance", "voting", "blockchain"]
    }
    ...
    rigor_indicators = ["formal", "proof", "theorem", "model", "framework", "analysis", "empirical", "systematic"]
    ...
    high_impact_terms = ["novel", "comprehensive", "survey", "systematic", "state-of-the-art", "sok"]
    citation_potential = 2 if any(term in abstract_lower for term in high_impact_terms) else 1

    # Component 4: Recency/Foundation (1-2 points)
    year = int(paper['submitted_date'][:4])
    # For foundational papers (2020-2021), give bonus; for recent papers (2024+), give bonus
    recency = 2 if (year <= 2021 or year >= 2024) else 1
...
def main():
    print("\n" + "="*60)
    print("QUERY SET 6: COMPLEMENTARY QUERIES")
    print("="*60)
    ...
    output_dir = Path("arxiv-sources/06-complementary/2024-2025")
